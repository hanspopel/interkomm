...
if [ "$BUILD_PROJECTS_INTERKOMM" == TRUE ]
	then
		:
	else
...
echo "#######      Configuring XCode Projects with TeamID: $CODESIGN_TEAM_ID_INTERKOMM BundleID: $CODESIGN_BUNDLE_ID_INTERKOMMå"
cmake -DDEPLOY:BOOL=$DEPLOY -DTEAMID:STRING=$CODESIGN_TEAM_ID_INTERKOMM -DBUNDLEID:STRING=$CODESIGN_BUNDLE_ID_INTERKOMM -DBUILD_NUMBER:STRING=$BUILD_NUMBER -DVERSION:STRING=$VERSION ../../source -GXcode 
